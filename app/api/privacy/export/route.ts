...
export async function GET() {
  try {
    ...
    const [
      ...
      // Reports submitted by other people about the user are not automatically
      // disclosed here because they can contain another person's confidential
      // information and may require a reviewed privacy-access process.
    ...
    if (await governanceTableExists("client_professional_links")) {
      governance.professionalConnections = await sql`
        SELECT * FROM client_professional_links
        WHERE client_user_id = ${user.id}
        ORDER BY created_at
      `
    }

    if (await governanceTableExists("sharing_grants")) {
      governance.sharingGrants = await sql`
        SELECT g.*
        FROM sharing_grants g
        JOIN client_professional_links l ON l.id = g.link_id
        WHERE l.client_user_id = ${user.id}
        ORDER BY g.created_at
      `
    }

    if (await governanceTableExists("consent_events")) {
      governance.consentEvents = await sql`
    ...
    if (await governanceTableExists("access_audit_events")) {
      governance.accessAuditEvents = await sql`
    ...
    if (await governanceTableExists("policy_acceptances")) {
      governance.policyAcceptances = await sql`
    ...
    if (await governanceTableExists("privacy_requests")) {
      governance.privacyRequests = await sql`
    ...
    const exportData = {
      ...
      community: {
        ...
        messagesAuthoredByUser: communityMessages,
        reportsSubmittedByUser: communityReportsMade,
        groupSwitchReasons,
      },
      governance,
    }
    ...
    return new NextResponse(JSON.stringify(exportData, null, 2), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="waypoint-data-export-${date}.json"`,
