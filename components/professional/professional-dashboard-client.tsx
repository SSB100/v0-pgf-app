...
import JourneyResponsesSummarySection, { type JourneyResponsesSummary } from "@/components/professional/journey-responses-summary"
...
type ProfessionalSummary = {
  ...
  journeyResponses?: JourneyResponsesSummary
  skillsPractice?: {
    period: SummaryPeriod
                    ...
                    <CardContent className="space-y-3 p-4">
                      {loading ? <p className="p-2 text-sm text-muted-foreground">Loading...</p> : clients.length === 0 ? <p className="p-2 text-sm text-muted-foreground">No connected clients yet.</p> : clients.map((client) => (
                        <div key={client.link_id} className={`rounded-lg border p-4 ${selectedClient?.link_id === client.link_id ? "border-primary/40 bg-primary/5" : ""}`}>
                          ...
                          <p className="mt-3 text-xs text-muted-foreground">Latest Daily Reflection: {client.latest_checkin_date ? dayLabel(client.latest_checkin_date) : "none shared yet"}</p>
                ...
                <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      <div className="space-y-2">{requestableDefinitions.map((scope) => <label key={scope.id} className="flex items-start gap-3 rounded-lg border p-3 text-sm"><Checkbox checked={inviteScopes.includes(scope.id)} onCheckedChange={(v) => toggleInviteScope(scope.id, v === true)} /><span><span className="flex flex-wrap items-center gap-2 font-medium">{scope.label}{scope.sensitivity === "high" && <Badge variant="outline" className="border-amber-500/40 text-amber-700 dark:text-amber-300">High sensitivity</Badge>}</span><span className="block text-muted-foreground">{scope.description}</span></span></label>)}</div>
...
function ClientSummaryCard({ client, summary, loading, windowDays, onWindowChange }: { client: ClientLink; summary: ProfessionalSummary | null; loading: boolean; windowDays: string; onWindowChange: (value: string) => void }) {
  return <Card>
    <CardHeader>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><CardTitle>{client.client_name || "Waypoint user"}</CardTitle><CardDescription>Structured self-reported information shared by this user.</CardDescription></div><div className="w-full sm:w-44"><p className="mb-2 text-xs font-medium text-muted-foreground">Daily Reflection window only</p><Select value={windowDays} onValueChange={onWindowChange}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="7">Last 7 days</SelectItem><SelectItem value="14">Last 14 days</SelectItem><SelectItem value="30">Last 30 days</SelectItem></SelectContent></Select></div></div>
    </CardHeader>
    <CardContent className="space-y-6">
      {loading ? <p className="text-sm text-muted-foreground">Loading authorised summary...</p> : !summary ? <p className="text-sm text-muted-foreground">Unable to display this summary.</p> : <>
        ...
        {summary.dailyCheckins && <SummarySection icon={CalendarDays} title="Daily Reflections" period={periodLabel(summary.dailyCheckins.period)}><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Daily Reflections" value={summary.dailyCheckins.summary.checkin_count ?? 0} /><Metric label="Average mood" value={summary.dailyCheckins.summary.average_mood ?? "No data"} /><Metric label="Average urge" value={summary.dailyCheckins.summary.average_urge ?? "No data"} /><Metric label="Gambling days" value={summary.dailyCheckins.summary.gambling_days ?? 0} /></div><p className="mt-3 text-xs text-muted-foreground">Missing days mean no Daily Reflection was recorded. They are not treated as symptom-free or gambling-free days.</p><div className="mt-3 space-y-2">{summary.dailyCheckins.trend.map((row, index) => <div key={`${row.date || "checkin"}-${index}`} className="grid gap-1 rounded-lg border px-3 py-2 text-xs sm:grid-cols-4"><span className="font-medium">{dayLabel(row.date)}</span><span>Mood {row.mood_rating ?? "-"}</span><span>Urge {row.urge_strength ?? "-"}</span><span>{row.gambling_occurred ? "Gambling reported" : "No gambling reported"}</span></div>)}</div></SummarySection>}
        ...
        {summary.journeyResponses && <JourneyResponsesSummarySection summary={summary.journeyResponses} />}
