...
export default function CommunityChatClient({
  ...
  const [reportedUser, setReportedUser] = useState<{ alias: string; userId: string; messageId?: string } | null>(
    null,
  )
  const [switchGroupDialogOpen, setSwitchGroupDialogOpen] = useState(false)
  const messagesEndRef = useRef<HTMLDivElement>(null)

  // Fetch messages and members on load
  useEffect(() => {
        ...
        <div className="bg-gradient-to-r from-primary/15 via-primary/10 to-transparent border-b-2 border-border/50 px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-2">
          <div className="space-y-0.5 min-w-0">
            ...
            <p className="text-xs sm:text-sm text-muted-foreground truncate">
              Anonymous • <span className="font-semibold text-foreground">{userAlias}</span>
            </p>
          </div>
          <Button
            variant="outline"
            size="sm"
            onClick={() => setSwitchGroupDialogOpen(true)}
            className="gap-1.5 border-primary/30 hover:bg-primary/10 flex-shrink-0 text-xs sm:text-sm px-2 sm:px-3"
          >
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground gap-3">
              ...
              <div>
                ...
                <p className="text-sm">Be the first to share encouragement and support!</p>
              </div>
            </div>
          ) : (
            messages.map((msg) => (
        ...
        <div className="border-t-2 border-border/50 bg-gradient-to-b from-card/50 to-card p-3 sm:p-4 space-y-2">
          <div className="flex gap-2">
            ...
            <Button 
              ...
              size="icon"
              className="bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary flex-shrink-0"
            >
              <Send className="w-4 h-4" />
            </Button>
          </div>
          <p className="text-xs text-muted-foreground/80 leading-relaxed hidden sm:block">
            Keep messages supportive and respectful. Your words matter.
          </p>
        </div>
      </div>

      {/* Sidebar - Members list (hidden on mobile) */}
