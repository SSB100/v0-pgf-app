...
export default function GuidedLearningModule({ module, moduleNumber, coreValues = [] }: GuidedLearningModuleProps) {
  ...
  const lessonSteps = Math.max(1, totalSteps - 1)
  ...
  const progressPercent = activeStep === 0 ? 0 : Math.round((activeStep / lessonSteps) * 100)
  const previewTopics = module.sections.slice(0, 3).map((section) => section.title)
  const extraTopics = Math.max(0, module.sections.length - previewTopics.length)
    ...
    <div className="min-h-screen bg-background pb-20">
      <main className="container mx-auto max-w-2xl px-4 py-5 sm:py-8 space-y-5">
          ...
          {activeStep > 0 && (
            <div className="mt-5 space-y-2">
              <div className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
                <span>Part {activeStep} of {lessonSteps}</span>
          ...
          <Card className="overflow-hidden border-primary/25 bg-gradient-to-br from-primary/10 via-card to-card shadow-sm">
            <CardContent className="p-5 sm:p-6 space-y-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">What this module is about</p>
                  <p className="mt-1.5 text-base leading-relaxed text-foreground/90">{module.whyItMatters}</p>
                </div>
              </div>

              <div className="rounded-xl border border-border/70 bg-background/70 p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">You will explore</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {previewTopics.map((topic) => (
                    <span key={topic} className="rounded-full border border-primary/20 bg-primary/7 px-3 py-1.5 text-xs font-medium text-foreground/85">
                      {topic}
                    </span>
                  ))}
                  {extraTopics > 0 && (
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-sm">How you will learn it</p>
                  <p className="mt-1 text-sm leading-relaxed text-foreground/80">
                    We will walk through {module.sections.length === 1 ? "one focused idea" : `${module.sections.length} focused ideas`} using everyday examples and visual aids where they help. Then you will do a quick learning check and a short reflection to connect the idea to real life.
                  </p>
                </div>
              </div>
        ...
        {learningSectionIndex >= 0 && (() => {
          ...
          const showValuesContext = learningSectionIndex === 0 && coreValues.length > 0 && (module.slug === "discovering-values" || module.slug === "values-to-action")
          ...
          return (
            <Card>
              ...
              <CardContent className="space-y-4">
                {showValuesContext && (
                  <div className="rounded-lg border border-pink-500/25 bg-pink-500/5 p-4">
