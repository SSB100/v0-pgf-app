...
export default function WeeklyOverviewCard({ checkins, journeyTypes = [] }: WeeklyOverviewCardProps) {
  ...
  const chartData = Array.from({ length: 7 }, (_, index) => {
    ...
    return {
      ...
      delta,
      feedback: !checkin ? "No check-in" : delta !== null && delta >= 2 ? "Positive shift" : delta !== null && delta <= -2 ? "A harder day — be kind to yourself" : "Steady day",
    }
  })

  const completedDays = chartData.filter((day) => day.hasData).length
  const missingDays = chartData.length - completedDays
  ...
  if (validCheckins.length === 0) {
    return (
      <Card className="soft-shadow border-border/50">
        <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base text-foreground sm:text-lg"><BarChart3 className="size-5 text-primary" />Weekly Overview</CardTitle></CardHeader>
        <CardContent className="pb-3"><div className="rounded-lg border-2 border-primary/20 bg-primary/10 p-3"><p className="mb-1 text-sm font-medium text-primary">Complete your first check-in to get started</p><p className="text-xs text-muted-foreground">Daily check-ins will appear here as separate bars for mood, overall rating, urges, and relapse status.</p></div></CardContent>
      </Card>
    )
  }
  ...
  return (
    <Card className="soft-shadow border-border/50">
      <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-base text-foreground sm:text-lg"><BarChart3 className="size-5 text-primary" />Weekly Overview<span className="ml-auto text-xs font-normal text-muted-foreground">{completedDays}/7 days</span></CardTitle></CardHeader>
      <CardContent className="space-y-3 pb-3">
        ...
        <ChartContainer config={chartConfig} className="h-[220px] w-full">
          <BarChart data={chartData} margin={{ top: 8, right: 4, left: -20, bottom: 0 }} barGap={1} barCategoryGap="18%">
            ...
            <Bar dataKey="mood" name="Mood" fill="var(--color-mood)" radius={[2, 2, 0, 0]}>{chartData.map((day) => <Cell key={`mood-${day.dateLabel}`} fill={day.hasData ? "var(--color-mood)" : "hsl(var(--muted))"} opacity={day.mood === null ? 0.25 : 1} />)}</Bar>
            <Bar dataKey="overall" name="Overall" fill="var(--color-overall)" radius={[2, 2, 0, 0]}>{chartData.map((day) => <Cell key={`overall-${day.dateLabel}`} fill={day.hasData ? "var(--color-overall)" : "hsl(var(--muted))"} opacity={day.overall === null ? 0.25 : 1} />)}</Bar>
            <Bar dataKey="urges" name="Urges" fill="var(--color-urges)" radius={[2, 2, 0, 0]}>{chartData.map((day) => <Cell key={`urges-${day.dateLabel}`} fill={day.hasData ? "var(--color-urges)" : "hsl(var(--muted))"} opacity={day.urges === null ? 0.25 : 1} />)}</Bar>
          </BarChart>
        </ChartContainer>

        <div className="grid grid-cols-7 gap-1" aria-label="Daily check-in status">
          {chartData.map((day) => <div key={day.dateLabel} className="flex min-w-0 flex-col items-center gap-1 text-center" title={day.feedback}><div className={`flex size-5 items-center justify-center rounded-full text-[10px] font-bold ${!day.hasData ? "bg-muted text-muted-foreground" : day.relapse ? "bg-destructive/15 text-destructive" : "bg-primary/15 text-primary"}`}>{day.hasData ? (day.relapse ? "!" : "✓") : "–"}</div>{day.delta !== null && day.delta >= 2 && <TrendingUp className="size-3 text-emerald-500" />}{day.delta !== null && day.delta <= -2 && <TrendingDown className="size-3 text-amber-500" />}</div>)}
        </div>
