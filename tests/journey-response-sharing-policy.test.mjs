...
test("migration creates private current-response storage and explicit Journey history mode", () => {
  ...
  assert.match(migration, /DROP CONSTRAINT IF EXISTS sharing_grants_data_scope_check_v2/)
  ...
  assert.match(migration, /DROP CONSTRAINT IF EXISTS sharing_grants_data_scope_check;[\s\S]*RENAME CONSTRAINT sharing_grants_data_scope_check_v2/)
  assert.match(migration, /DROP CONSTRAINT IF EXISTS sharing_grants_journey_history_check_v2/)
  assert.match(migration, /ADD CONSTRAINT sharing_grants_journey_history_check_v2[\s\S]*NOT VALID/)
  assert.match(migration, /VALIDATE CONSTRAINT sharing_grants_journey_history_check_v2/)
  assert.match(migration, /DROP CONSTRAINT IF EXISTS sharing_grants_journey_history_check;[\s\S]*RENAME CONSTRAINT sharing_grants_journey_history_check_v2/)
})
